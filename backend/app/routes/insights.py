...
async def get_insights(request: Request, body: InsightsRequest) -> InsightsResponse:
    ...
    if settings.USE_GEMINI:
        try:
            ...
            raw_insights = _apply_rule_engine(result, body.inputs)
            source = "rules"
    else:
        raw_insights = _apply_rule_engine(result, body.inputs)
        source = "rules"
    ...
    if settings.USE_BIGQUERY:
        diet_type_logged = body.inputs.diet_type if body.inputs else "unknown"
        asyncio.create_task(
            bigquery_service.log_event_async(
                total_kg=result.total_kg,
                diet_type=diet_type_logged,
...
def _apply_rule_engine(result: CarbonResult, inputs: CarbonInput | None = None) -> list[InsightItem]:
    ...
    if inputs:
        ...
        flights_short = inputs.flights_short_haul
        flights_long = inputs.flights_long_haul
    else:
        # Fallback placeholder defaults if inputs not provided by old client
